...
const setCreditClass = async (req, res, next) => {
  ...
  try {
    ...
    studentHasSubject = await StudentHasSubject.findByPk(
      score.studentHasSubjectId
    )
    req.creditClass = await CreditClass.findByPk(
      studentHasSubject.creditClassId
    )
...
router.get('/list-student/:creditClassId',
  setCreditClass,
  authUser,
  authGetCreditClass,
  async (req, res, next) => {
    ...
    try {
      let listStudentCreditClass = await userModel.findAll({
        include: {
          model: studentModel,
          required: true,
          include: {
      ...
      if (listStudentCreditClass.length === 0) {
        return res.status(404).json({
          error: { message: 'Không có sinh viên nào đăng kí lớp học' },
        })
      } else {
...
router.get('/scores/me',
  ...
  async (req, res, next) => {
    try {
      ...
      listStudentCreditClass = await StudentHasSubject.findAll({
      ...
      if (scoresOfSubject.length === 0)
        return res.status(202).send({
          error: {
            message: 'Sinh viên chưa tham gia bất cứ lớp tín chỉ nào',
          },
        })
